...
public class CameraManager: NSObject, AVCaptureFileOutputRecordingDelegate, UIGestureRecognizerDelegate {
    ...
    public func hasFlash() -> Bool {
        ...
        for  device in devices  {
            ...
            if (captureDevice.position == .Back && cameraDevice == .Back) {
                return captureDevice.hasFlash
            } else if(captureDevice.position == .Front && cameraDevice == .Front) {
                return captureDevice.hasFlash
            }
        }
     ...
     */
    public func askUserForCameraPermission(completion: (permissionVideo:Bool, permissionMic:Bool?) -> Void) {
        
        AVCaptureDevice.requestAccessForMediaType(AVMediaTypeVideo, completionHandler: {
            (alowedAccess) -> Void in
            
            var allowedVideo = alowedAccess
            
            if self.cameraOutputMode == .VideoWithMic {
                AVCaptureDevice.requestAccessForMediaType(AVMediaTypeAudio, completionHandler: {
                    (alowedAccess) -> Void in
                    
                    var allowedMic = alowedAccess
                    
                    dispatch_sync(dispatch_get_main_queue(), { () -> Void in
                        completion(permissionVideo: allowedVideo, permissionMic: allowedMic)
                    })
                })
            } else {
                dispatch_sync(dispatch_get_main_queue(), { () -> Void in
                    completion(permissionVideo: alowedAccess, permissionMic: nil)
                })
            }
        })
    }
